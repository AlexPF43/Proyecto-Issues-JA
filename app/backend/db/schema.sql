...
DROP TABLE IF EXISTS custom_state_mappings;
...
CREATE TABLE `tickets` (
  ...
  `status` varchar(255) DEFAULT NULL COMMENT 'Estado original del ticket',
  `mapped_status` varchar(255) DEFAULT NULL COMMENT 'Estado padre normalizado',
  ...
  KEY `idx_tickets_mapped_status` (`mapped_status`)
) ENGINE=InnoDB AUTO_INCREMENT=5281 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `custom_state_mappings` (
  `id` int NOT NULL AUTO_INCREMENT,
  `source` varchar(50) NOT NULL,
  `parent_state` varchar(255) NOT NULL,
  `custom_state` varchar(255) NOT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_source_custom_state` (`source`, `custom_state`),
  KEY `idx_custom_state_source` (`source`),
  KEY `idx_custom_state_parent` (`parent_state`)
